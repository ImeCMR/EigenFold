...
class HarmonicSDE:
    ...
    def eigens(self, t): # eigenvalues of sigma_t
        ...
        D = 1/(self.D + 1e-8) * (1 - np_.exp(-t*self.D))
    ...
    def rmsd(self, t):
        l = self.l
        D_sliced = self.D[l:]
        D = 1/(D_sliced + 1e-8) * (1 - np.exp(-t*D_sliced))
        return np.sqrt(3*D.mean())
    ...
    def KL_H(self, t):
        ...
        D = self.D[l:]
        eps = 1e-8
        return -3*0.5*(np.log(1-np.exp(-D*t) + eps)+np.exp(-D*t)).sum(0)
    ...
    def get_x_0_from_xt_and_score(self, xt, score, t, center=True):
        if self.use_cuda:
            return self._get_x_0_from_xt_and_score_torch(xt, score, t, center)
        else:
            xt_np = xt.cpu().numpy()
            score_np = score.cpu().numpy()
            x0_np = self._get_x_0_from_xt_and_score_numpy(xt_np, score_np, t, center)
            return torch.from_numpy(x0_np).to(xt.device)

    def _get_x_0_from_xt_and_score_numpy(self, xt, score, t, center=True):
        D_t = self.eigens(t)

        xt_proj = self.P.T @ xt
        score_proj = self.P.T @ score
        if center:
            score_proj[0] = 0

        x0_proj = np.exp(t * self.D / 2)[:, None] * (xt_proj + D_t[:, None] * score_proj)

        x0 = self.P @ x0_proj
        return x0

    def _get_x_0_from_xt_and_score_torch(self, xt, score, t, center=True):
        D_t = self.eigens(t)
        ...
        x0_proj = torch.exp(t * self.D.unsqueeze(-1) / 2) * (xt_proj + D_t.unsqueeze(-1) * score_proj)

...
def loss_func(data, args, sde):
    loss = ((data.score - data.pred)**2 / data.score_norm[:,None]**2).mean()
    base_loss = (data.score**2 / data.score_norm[:,None]**2).mean()

    if 'nmr' in data and hasattr(data['nmr'], 'edge_index') and data['nmr'].edge_index.size(1) > 0:
        xt = data['resi'].pos
        score = data.pred * data.score_norm[:, None]
        t = data['resi'].node_t[0].item()

        x0_pred = sde.get_x_0_from_xt_and_score(xt, score, t)

        nmr_edge_index = data['nmr'].edge_index
        nmr_dist_target = data['nmr'].dist
        src, dst = nmr_edge_index

        dist_pred = (x0_pred[src] - x0_pred[dst]).norm(dim=-1)

        nmr_loss = torch.nn.functional.huber_loss(dist_pred, nmr_dist_target, delta=args.nmr_huber_delta)

        total_loss = loss + args.nmr_loss_weight * nmr_loss

        return total_loss, base_loss, nmr_loss
    else:
        return loss, base_loss, torch.tensor(0.0).to(data.pred.device)
...
def epoch(args, model, loader, optimizer=None, scheduler=None, device='cpu', print_freq=1000):
    ...
    sde = loader.dataset.sde
    if not args.wandb: loader = tqdm.tqdm(loader, total=len(loader))
    log = {'rmsd': [], 'step': [], 'loss': [], 'base_loss': [], 'nmr_loss': []}
    for i, data in enumerate(loader):
        ...
        try:
            if args.data_skip and data.skip:
                ...
                continue
            data, loss, base_loss, nmr_loss = iter_(model, data, optimizer, args, sde)
            ...
            with torch.no_grad():
                ...
                log['nmr_loss'].append(float(nmr_loss.cpu().numpy()))
...
def iter_(model, data, optimizer, args, sde):
    if optimizer is not None:
        model.zero_grad()
        pred = model(data)
        data.pred = pred
        loss, base_loss, nmr_loss = loss_func(data, args, sde)
        loss.backward()
        ...
        with torch.no_grad():
            ...
            data.pred = pred
            loss, base_loss, nmr_loss = loss_func(data, args, sde)
    return data, loss, base_loss, nmr_loss

...
from diffusion.sde import HarmonicSDE as PolymerSDE
...
class ResidueDataset(Dataset):
    def __init__(self, args, split, **kwargs):
        ...
        self.args = args
        self.noise_std_dev = 0.1 # For NMR noise
    ...
    def get(self, idx):
        ...
        # On-the-fly NMR data generation
        if 'pos' in data['resi']:
            ilv_residues = []
            for i, res_type in enumerate(row.seqres):
                if res_type in ['I', 'L', 'V']:
                    ilv_residues.append(i)

            restraints = []
            for i in range(len(ilv_residues)):
                for j in range(i + 1, len(ilv_residues)):
                    res_idx1 = ilv_residues[i]
                    res_idx2 = ilv_residues[j]

                    if abs(res_idx1 - res_idx2) >= 4:
                        ca_pos1 = data['resi'].pos[res_idx1]
                        ca_pos2 = data['resi'].pos[res_idx2]
                        dist = torch.norm(ca_pos1 - ca_pos2).item()

                        if dist <= 8.0:
                            noisy_dist = dist + np.random.normal(0, self.noise_std_dev * dist)
                            restraints.append({'res1': res_idx1, 'res2': res_idx2, 'dist': noisy_dist})

            if restraints:
                edge_index = torch.tensor([[r['res1'], r['res2']] for r in restraints]).t().long()
                dist = torch.tensor([r['dist'] for r in restraints]).float()
                data.nmr = HeteroData()
                data.nmr.edge_index = edge_index
                data.nmr.dist = dist

        if self.args.embeddings_key == 'name':
            embeddings_name = row['name']
        else:

...
def main():
    ...
    noesy_restraints = None
    if inf_args.noesy_restraints:
        noesy_restraints = parse_noesy_restraints(inf_args.noesy_restraints)

    val_loader = get_loader(args, None, splits, mode=inf_args.split_key, shuffle=False)
    samples, log = inference_epoch(args, model, val_loader.dataset, device=device, pdbs=True, elbo=inf_args.elbo, noesy_restraints=noesy_restraints)
...
def parse_noesy_restraints(filepath):
    restraints = []
    with open(filepath, 'r') as f:
        for line in f:
            ...
            parts = line.split()
            restraints.append({
                'res1_num': int(parts[0]),
                'res2_num': int(parts[1]),
